<template>
    <div class="wrap">
        ...
        <div class="places">
            ...
            <div class="place" :data-placenum="index" v-for="(place, index) in places" :key="place.name">
                <div class="place-label">{{index + 1}}.</div>
                <div class="place-name">{{place.name}}</div>
                <div class="place-points">({{place.rating}} točk)</div>
            </div>
...
export default {
    ...
    methods: {
        ...
        ...mapActions(['fetchLeaderboardTest']),

        reloadData() {
            Promise.all([
                this.fetchLeaderboardTest().then((d) => {
                    this.places = [d[0], d[1], d[2]];
                    this.ddd.leaderboard = d.filter((c, idx, a) => {return idx > 2;}); 
                })
            ]).then(() => {
        ...
        columnRename(colName) {
            ...
            else if(colName == "name") return "Ime";
            else if(colName == "rating") return "Točke";
            else if(colName == "leaderboard") return "Mesto na lestvici";
